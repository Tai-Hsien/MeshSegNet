...
class MeshSegNet(nn.Module):
    def __init__(self, num_classes=15, num_channels=15, with_dropout=True, dropout_p=0.5):
        ...
        self.mlp1_bn = nn.BatchNorm1d(64, track_running_stats=False)
        # FTM (feature-transformer module)
        self.fstn = STNkd(k=64)
        # GLM-1 (graph-contrained learning modulus)
        self.glm1_conv1 = torch.nn.Conv1d(64, 32, 1)
        self.glm1_bn1 = nn.BatchNorm1d(32, track_running_stats=False)
        self.glm1_conv2 = torch.nn.Conv1d(32+32, 64, 1)
        self.glm1_bn2 = nn.BatchNorm1d(64, track_running_stats=False)
        # MLP-2
        self.mlp2_conv1 = torch.nn.Conv1d(64, 64, 1)
        self.mlp2_bn1 = nn.BatchNorm1d(64, track_running_stats=False)
        self.mlp2_conv2 = torch.nn.Conv1d(64, 128, 1)
        self.mlp2_bn2 = nn.BatchNorm1d(128, track_running_stats=False)
        self.mlp2_conv3 = torch.nn.Conv1d(128, 512, 1)
        self.mlp2_bn3 = nn.BatchNorm1d(512, track_running_stats=False)
        # GLM-2 (graph-contrained learning modulus)
        self.glm2_conv1 = torch.nn.Conv1d(512, 128, 1)
        self.glm2_bn1 = nn.BatchNorm1d(128, track_running_stats=False)
        self.glm2_conv2 = torch.nn.Conv1d(128*3, 512, 1)
        self.glm2_bn2 = nn.BatchNorm1d(512, track_running_stats=False)
        # MLP-3
        self.mlp3_conv1 = torch.nn.Conv1d(64+512+512+512, 256, 1)
        self.mlp3_conv2 = torch.nn.Conv1d(256, 256, 1)
        self.mlp3_bn1 = nn.BatchNorm1d(256, track_running_stats=False)
        self.mlp3_conv3 = torch.nn.Conv1d(256, 128, 1)
        self.mlp3_conv4 = torch.nn.Conv1d(128, 128, 1)
        self.mlp3_bn2 = nn.BatchNorm1d(128, track_running_stats=False)
        # output
    ...
    def forward(self, x, a_s, a_l):
        ...
        n_pts = x.size()[2]
        # MLP-1
        x = F.relu(self.mlp1_bn(self.mlp1_conv1(x)))
        x = F.relu(self.mlp1_bn(self.mlp1_conv2(x)))
        # FTM
        ...
        glm_1_sap = F.relu(self.glm1_bn1(self.glm1_conv1(sap)))
        ...
        glm_2_sap_1 = F.relu(self.glm2_bn1(self.glm2_conv1(sap_1)))
        glm_2_sap_2 = F.relu(self.glm2_bn1(self.glm2_conv1(sap_2)))
        ...
        # MLP-3
        x = F.relu(self.mlp3_bn1(self.mlp3_conv1(x)))
        x = F.relu(self.mlp3_bn1(self.mlp3_conv2(x)))
        x = F.relu(self.mlp3_bn2(self.mlp3_conv3(x)))
        if self.with_dropout:
            x = self.dropout(x)
        x = F.relu(self.mlp3_bn2(self.mlp3_conv4(x)))
        # output
